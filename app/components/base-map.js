...
export default class BaseMap extends Component {
  ...
  onPressSearchButton = () => {
    if (this.state.searchText.length > 0) {
      axios
        .get(`${HOST}:${PORT}/api/map/geocode/forward`, {
          params: {
        ...
        .then(res => {
          ...
          const filteredAnnotations = this.state.annotations.filter(
            annotation => {
              return (
                annotation.id !== 'search' && annotation.id !== 'directions'
              );
            }
          );
    ...
    if (this.state.showCrimes) {
      ...
      this.setState({
        annotations: this.state.annotations.filter(annotation => {
          return (
            annotation.title === 'Marked Unsafe' ||
            annotation.id === 'search' ||
    ...
    this.setState(
      ...
      () => {
        if (this.state.showCrimes) {
          ...
          const absoluteLat = Math.abs(
            location.latitude - this.state.crimesLocation.latitude
          );
          const absoluteLon = Math.abs(
            location.longitude - this.state.crimesLocation.longitude
          );
          // If location changed by more than .0005 degrees
          if (absoluteLat > 0.0005 || absoluteLon > 0.0005) {
  ...
  onRightAnnotationTapped = selectedPoint => {
    ...
    if (selectedPoint.id === 'search' && !this.state.showDirections) {
      ...
      axios
        .get(`${HOST}:${PORT}/api/map/directions`, {
          params: {
            start: `${this.state.userLocation.longitude},${this.state
      ...
      this.setState({
        annotations: this.state.annotations.filter(
          annotation => annotation.id !== 'directions'
        ),
        showDirections: !this.state.showDirections
      });
    } else {
  ...
  onLongPress = location => {
    ...
    axios
      .get(`${HOST}:${PORT}/api/map/geocode/reverse`, {
        params: {
          latitude: location.latitude,
          longitude: location.longitude
        }
          ...
          annotations: [
            ...this.state.annotations,
            {
              ...
              annotationImage: {
                source: {
                  uri:
                    'http://www.freeiconspng.com/uploads/emergency-alert-icon-alert-icon-8.png'
                },
                height: 25,
                width: 25
              },
              rightCalloutAccessory: {
                source: {
                  uri:
                    'https://openclipart.org/image/2400px/svg_to_png/16155/milker-X-icon.png'
                },
                height: 15,
                width: 15
              },
    ...
    if (this.state.showDirections) {
      ...
      axios
        ...
        .then(res => {
          ...
          SendSMS.send(
            {
              body: `Hey, I'm currently at ${address
                .slice(0, 3)
                .join(',')
                .toUpperCase()}. I'm walking to ${destination
                .slice(0, 3)
                .join(',')
                .toUpperCase()}.`,
        ...
        <View style={mapStyle.mapButtons}>
          <View style={mapStyle.buttonsLeft}>
            <View style={mapStyle.alert}>
              <Text onPress={() => this.sendLocationToContacts()}>
                {alertIcon}
              </Text>
            </View>
          </View>
            ...
            <TouchableHighlight style={mapStyle.crimeView}>
              <View>
                {!this.state.showCrimes && (
                  <Text onPress={() => this.onCrimesToggleClick()}>
                    {noViewIcon}
                  </Text>
                )}
                {this.state.showCrimes && (
